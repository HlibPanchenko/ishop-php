...
class AppModel extends Model
{
    ...
    public static function create_slug($table, $field, $str, $id): string
    {
        ...
        // в таблице category ищем поле slug =  $str
        $res = R::findOne($table, "$field = ?", [$str]);
        if ($res) {
            // к этому слагу пристыковываем id категории
            $str = "{$str}-{$id}";
            // потворно достаем уже с пристыкованым id
            $res = R::count($table, "$field = ?", [$str]);
            // если такой слаг уже есть, то рекурсивно вызываем метод
            // добавится еще один id чтобы уже навернека слаг был уникальным
            if ($res) {
                $str = self::create_slug($table, $field, $str, $id);
            }
        }
        // возвращает уникальный слаг категории
        return $str;
    }
