...
class Page extends AppModel
{
    ...
    {
        return R::getAll("SELECT p.*, pd.title FROM page p JOIN page_description pd on
    p.id = pd.page_id WHERE pd.language_id = ? LIMIT $start, $perpage", [$lang['id']]);
    }
    ...
    public function deletePage($id): bool
    {
        R::begin(); // транзакция
        try {
            $page = R::load('page', $id); // получаем бин
            if (!$page) {
                return false;
            }
            R::trash($page); // удаляем
    ...
    public function save_page(): bool
    {
        ...
        R::begin();
        try {
            //  создания новой записи в таблице page
            $page = R::dispense('page');
            $page_id = R::store($page);
            // генерим уникальный слаг
    ...
    {
        return R::getAssoc("SELECT pd.language_id, pd.*, p.* FROM page_description pd JOIN
    page p on p.id = pd.page_id WHERE pd.page_id = ?", [$id]);
    }
}