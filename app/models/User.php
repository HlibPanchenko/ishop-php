...
class User extends AppModel
{
    ...
    public function login($is_admin = false): bool
    {
        ...
        if ($email && $password) {
            // используем для авторизации и для обычных юзеров и для админов
            if ($is_admin) {
            ...
            if ($user) {
                ...
                if (password_verify($password, $user->password)) {
                    foreach ($user as $k => $v) {
                        if (!$k != 'password') {
                            // в сессию записываем все данные пользователя, кроме пароля
                            $_SESSION['user'][$k] = $v;
                        }
                    }
                    return true;
    ...
    {
        return R::getAll("SELECT * FROM orders WHERE user_id = ? ORDER BY id DESC LIMIT
                                                $start, $perpage", [$user_id]);
    }

    public function get_user_order($id): array
    {
        return R::getAll("SELECT o.*, op.* FROM orders o JOIN order_product op on
    o.id = op.order_id WHERE o.id = ?", [$id]);
    }

    public function get_count_files(): int
    {
        return R::count('order_download', 'user_id = ? AND status = 1',
            [$_SESSION['user']['id']]);
    }

    public function get_user_files($start, $perpage, $lang): array
    {
        return R::getAll("SELECT od.*, d.*, dd.* FROM order_download od JOIN
    download d on d.id = od.download_id JOIN download_description dd on
        d.id = dd.download_id WHERE od.user_id = ? AND od.status = 1 AND
                                    dd.language_id = ? LIMIT $start, $perpage",
    ...
    public function get_user_file($id, $lang): array
        // $id - id файла
    {
        return R::getRow("SELECT od.*, d.*, dd.* FROM order_download od JOIN
    download d on d.id = od.download_id JOIN download_description dd on
        d.id = dd.download_id WHERE od.user_id = ? AND od.status = 1 AND
                                    od.download_id = ? AND dd.language_id = ?",
