...
{
    // вот так надо обращаться к этой странице - login-admin => loginAdmin
    // это мы прописывали когда писали класс Роутера
    ...
    public function editAction()
    {
        ...
        if (!empty($_POST)) {
            $this->model->load();
            // если не отправили пароль, чтобы пустая строка не попадала в валидациб, просто удаялем attributes['password']
    ...
    public function loginAdminAction()
    {
        if ($this->model::isAdmin()) {
            // если уже залогинен как админ, то редирект на главную страницу админки
            redirect(ADMIN);
        }
        ...
        if (!empty($_POST)) {
            ...
            // если в сессии появились данные о пользователе, значит он авторизован
            if ($this->model::isAdmin()) {
