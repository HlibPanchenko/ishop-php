...
class DownloadController extends AppController
{
    ...
    public function indexAction()
    {
        ...
        $page = get('page');
        $perpage = 20;
        $total = R::count('download'); // к-во файлов
...
//        debug($downloads, 1);
    ...
    public function addAction()
    {
        if (!empty($_POST)) {
            if ($this->model->download_validate()) {
                // в $data должен попасть массив инфы о загруженном файле
    ...
    {
        $id = get('id');
        // Можно удалять только те файлы, которые не приобретались
        if (R::count('order_download', 'download_id = ?', [$id])) {
            $_SESSION['errors'] = 'Невозможно удалить - данный файл уже приобретался';
            redirect();
        }
        // если файл прикреплен к товару, то файл нельзя его удалить
